...
/*   Updated: 2022/11/19 10:55:35 by seojin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm( Form const &f ) const
{
	try
	{
		if (f.execute(*this) == false)
			throw Bureaucrat::PermisionDenyed();
		std::cout << this->getName() << " executed " << f.getName() << std::endl;
	}
	catch (const PermisionDenyed &pd )
	{
		pd.print();
		std::cout << this->getName() << " can't execute " << f.getName() << " form." << std::endl;
	}
...
void Bureaucrat::PermisionDenyed::print( void ) const
{
	std::cout << "Permision denyed." << std::endl;
}
